...
void init_request(Request*, C_Request*, int, bool);
void find_k_shortest_paths(Request, int, ostream&, ostream&);
void find_k_shortest_paths_for_new_requests(Request, int, int, ostream&, ostream&);
...
//int main(int argc, char* argv[]) {
	...
	/* initialization of new substrate network for the control group */
	C_Phy_Topo *C_Topo;
...
/* ======================================== embedded requests (background) ======================================== */
	...
	init_request(embedded_request, c_embedded_request, n_embedded_request, true);
...
/* ======================================== new requests ======================================== */
	...
	init_request(request, c_request, n_request, false);

	/*
	// Trace
	for (int i=0; i<n_request; i++) {
		int f_num = request[i].flow_count;	
		cout << "request[" << i << "]\ttotal_mean\t" << request[i].total_mean << endl;	
		cout << endl;	
		for (int j=0; j<f_num; j++) {
			cout << "flow[" << j << "]\t" << request[i].flows[j]->mean2 << "\t";	
		}
		cout << endl;	
		cout << endl;	
	}
	...
	// Trace
	for (int i=0; i<n_request; i++) {
		int f_num = c_request[i].c_flow_count;	
		cout << "c_request[" << i << "]\ttotal_demand\t" << c_request[i].total_demand << endl;	
		cout << endl;	
		for (int j=0; j<f_num; j++) {
			cout << "flow[" << j << "]\t" << c_request[i].c_flows[j]->demand2 << "\t";	
		}
		cout << endl;	
		cout << endl;	
	}
	...
	*/
	...
	for (int i=0; i<n_request; i++) {
		...
		out_c_new_File << "c_new_request[" << i << "]" << endl;
		...
		for (int j=0; j<flow_number; j++) {
			...
			find_k_shortest_paths_for_new_requests(request[i], j, top_k, out_new_File, out_c_new_File);
		}
	}
	...
	while (!in_new_File.eof()) {
		...
		if (*node_arr_for_path == 'n') {
			path_count = 0;	
			flow_count = 0;	
			user_count++;
		}
		else if (*node_arr_for_path == '[') {
			flow_count++;
			path_count = 0;	
		}
		else if (*node_arr_for_path == 'c') {
		...
		try {
			...
			for (int j=0; j<MAX_FLOW_SIZE; j++) {
				for (int k=0; k<top_k; k++) {
					for (int l=0; l<SEDGENUM; l++) {
						if (j<c_flow_number)	
							c_obj += c_request[i].c_flows[j]->demand2 * c_path_allocator[j][k] * link_indicator[i][j][k][l];
						else {
							// do nothing
						}
					}
			...
			for (int l=0; l<SEDGENUM; l++) {
				...
				for (int j=0; j<c_flow_number; j++) {
					for (int k=0; k<top_k; k++) {
					//	cout << "k: " << k << ", i: " << i << ", j: " << j << ", value: " << c_request[i].demand * link_indicator[i][j][k] << endl;	
						c_link_constraint_expr += c_request[i].c_flows[j]->demand2 * c_path_allocator[j][k] * link_indicator[i][j][k][l];	
					}
				}
...
/* ======================================== Tracing ======================================== */
	...
	if (trace_on == true) {
		...
		cout << "# of new accepted requests\t" << accepted_count << "\ttotal_new_requests\t" << n_request << endl;
		if (accepted_count == n_request) {
			cout << "!\t" << 1 << endl;
			acc_req_result_File << "!\t" << 1 << endl;	
		}
		else {
			cout << "!\t" << 0 << endl;
			acc_req_result_File << "!\t" << 0 << endl;	
		}
		cout << endl;

		cout << "********** control group **********" << endl << endl;
		accepted_count = 0;	
		for (int i=0; i<n_request; i++) {
		...
		if (accepted_count == n_request) {
			cout << "@\t" << 1 << endl;
			c_acc_req_result_File << "@\t" << 1 << endl;	
		}
		else {
			cout << "@\t" << 0 << endl;
			c_acc_req_result_File << "@\t" << 0 << endl;	
		}
		cout << endl;
		cout << endl;

		acc_req_result_File << "the end" << endl;	
		c_acc_req_result_File << "the end" << endl;	
...
void init_request(Request* request, C_Request* c_request, int n_request, bool accepted) {
	int node1;
	int node2;
...
void find_k_shortest_paths_for_new_requests(Request request, int flow_number, int top_k, ostream& out_stream, ostream& out_stream2) {
	Graph my_graph("B4_SN");
	vector<BasePath*> result;
	vector<BasePath*>::iterator result_iter;
