...
#define CAPACITY_LIMIT 500	// cn(l)/C-c(l)
				...
				switch (alg) {
					...
					case Strong_Var:
						for (int l=0; l<SEDGENUM; l++) {
							...
							//link_constraint_expr = (0.120406) - (0.189989) * (((link_weight - link_constraint_expr_mean) / sqrt(constraint_expr_var)) - 1.281);
//							link_constraint_expr = (approximation_constant) - (approximation_slope) * (((link_weight - link_constraint_expr_mean) / sqrt(constraint_expr_var)) - expansion_point);
							link_constraint_expr =(-1)*(approximation_slope)*(((link_weight - link_constraint_expr_mean) / sqrt(constraint_expr_var))-expansion_point)+link_min_eps;
//							link_constraint_expr =-5*(expansion_point)*(expansion_point)/24 +1/3-5*(expansion_point) * (((link_weight - link_constraint_expr_mean) / sqrt(constraint_expr_var)) - expansion_point)/12;
