...
#define SEDGENUM 26		// number of edges: 19
#define CAPACITY 20	// capacity: 160Gbps
...
//	srand(time(NULL));
	...
	int n_request = 150;	// the number of new requests
...
*/
	...
	if (trace_on == true) {
		...
		for (int l=0; l<SEDGENUM; l++) {
			mean_utilization += Topo->edges[l]->filled_weight;
		}
		cout << "# of new accepted requests\t" << accepted_count << "\ttotal_new_requests\t" << n_request << "\treq_qos\t" << req_qos << endl;
		cout << "accepted_mean\t" << accepted_mean << "\tmean_utilization\t" << mean_utilization << endl;	
		...
		acc_req_result_File << accepted_count << "\t" << n_request << "\t" << min_qos << "\t" << violate_constraints << "\t" << req_qos << "\t" << accepted_mean << "\t" << mean_utilization << endl;
...
/*				if (relax_debug == true) {
					...
					//_mean = double(rand() % 3 + 3);	
					//_stddev = double(rand() % 2 + 1);	
					_mean = 4.0;
					_stddev = 2.0;
					_demand = _mean + _stddev * normsinv(double(1.0 - _eps));
